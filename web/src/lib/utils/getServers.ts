...
const isBotInGuild = async (guild: DiscordGuild) => {
  const rawResp = await fetch(`${import.meta.env.VITE_DISCORD_API_URL}/guilds/${guild.id}/members/${import.meta.env.VITE_DISCORD_CLIENT_ID}`, {
    method: 'GET',
    headers: {
...
export const getServers = async (token: string | undefined) => {
  ...
  try {
    const rawResp = await fetch(`${import.meta.env.VITE_DISCORD_API_URL}/users/@me/guilds`, {
      method: 'GET',
      headers: {
